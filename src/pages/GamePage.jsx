...
import { getMovable, checkCapture, checkWinner, COLOR_HEX, formatTime, PATH, HOME_STRETCH, calcNewPos } from '../utils/gameLogic';
    ...
    <div onClick={canRoll && isActive && !rolling ? onRoll : undefined}
      ...
      <svg viewBox="0 0 100 100" width={size} height={size}
        ...
        <rect width="100" height="100" rx="20"
          ...
          stroke={hex} strokeWidth={isActive ? 5 : 2}
          opacity={isActive ? 1 : 0.3}/>
        {isActive && rolling &&
          <text x="50" y="68" textAnchor="middle" fontSize="48" fill={hex} fontWeight="900">?</text>}
        {isActive && !rolling && dots.map(([dx,dy],i) =>
          <circle key={i} cx={dx} cy={dy} r="11" fill={value===6 ? hex : '#222'}/>)}
        {isActive && !rolling && !value && canRoll &&
          <text x="50" y="64" textAnchor="middle" fontSize="34" fill={hex}>🎲</text>}
        {!isActive &&
          <rect x="20" y="44" width="60" height="12" rx="6"
            fill={hex} opacity=".2"/>}
      </svg>
      {isActive && canRoll && !rolling && !value &&
        <div style={{fontSize:9,fontWeight:900,color:hex,letterSpacing:1.5,
          animation:'pulse 1s ease infinite'}}>TAP!</div>}
      {isActive && value && !rolling &&
        <div style={{fontSize:16,fontWeight:900,color:hex,lineHeight:1}}>{value}</div>}
    </div>
  );
}
...
function Toast({msg,type}) {
  ...
  return (
    <div style={{
      ...
      fontFamily:'Rajdhani',letterSpacing:.4,pointerEvents:'none',
...
function EmojiBurst({burst}) {
  ...
  return (
    <div style={{position:'fixed',top:'50%',left:'50%',
      ...
      <div style={{fontSize:84,lineHeight:1,filter:`drop-shadow(0 0 18px ${hex})`,
        animation:'popIn .4s cubic-bezier(.34,1.56,.64,1)'}}>{burst.emoji}</div>
      <div style={{fontFamily:'Rajdhani',fontWeight:800,fontSize:14,color:hex,
        marginTop:5,letterSpacing:2}}>{burst.name}</div>
    </div>
  );
}
  ...
  */
  ...
  const doMove = useCallback((color, idx, newPos) => {
    ...
    animateMove(color, idx, fromPos, newPos, () => {
      ...
      const nt={...tokens,
        [color]:tokens[color].map((t,i)=>i===idx?{...t,pos:isHome?57:newPos,finished:isHome}:t)};
  ...
  const corners={};
  players.forEach(p=>{corners[COLOR_CORNER[p.color]]=p;});
  ...
  const Corner = ({ pos, align }) => {
    ...
    if (!p) return <div/>;
    const alignStyles = {
      tl: { alignItems:'flex-end',   justifyContent:'flex-end',   padding:'0 5px 5px 0' },
      tr: { alignItems:'flex-end',   justifyContent:'flex-start',  padding:'0 0 5px 5px' },
      bl: { alignItems:'flex-start', justifyContent:'flex-end',   padding:'5px 5px 0 0' },
      br: { alignItems:'flex-start', justifyContent:'flex-start',  padding:'5px 0 0 5px' },
    };
    return (
      <div style={{ display:'flex', ...alignStyles[pos] }}>
        <CornerWidget
          player={p} tokens={tokens}
          isActive={players[currentTurn]?.color === p.color}
          diceValue={diceValue} diceRolling={diceRolling}
          canRoll={canRoll} onRoll={handleRoll}
          onEmoji={handleEmoji} lastEmoji={emojiMap[p.color]}
          position={pos}
        />
      </div>
    );
    ...
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', gap:0 }}>
        ...
        {players.map(p => {
          ...
          return (
            <div key={p.id} style={{
              ...
              background: isAct ? `${Z[p.color]}22` : 'rgba(255,255,255,0.07)',
              border:`1.5px solid ${isAct ? Z[p.color] : 'rgba(255,255,255,0.12)'}`,
              boxShadow: isAct ? `0 0 10px ${Z[p.color]}44` : 'none',
              transition:'all .3s',
            }}>
              <div style={{width:8,height:8,borderRadius:'50%',background:Z[p.color],
                boxShadow:`0 0 5px ${Z[p.color]}`}}/>
              <span style={{fontFamily:'Rajdhani',fontWeight:700,fontSize:12,color:'#ddd'}}>{p.username}</span>
              <span style={{fontFamily:'Rajdhani',fontWeight:900,fontSize:13,color:Z[p.color]}}>{home}/4</span>
            </div>
          );
        })}
      </div>
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{borderRadius:9,overflow:'hidden',background:'#b0b8c8',width:'100%',height:'100%'}}>
            <LudoBoard
              tokens={animTokens}
              players={players}
  ...
  return (
    <div style={{
      minHeight:'100vh',
      background:'linear-gradient(160deg,#2c2f4a 0%,#3a3d5c 40%,#2a3050 100%)',
      ...
      <div style={{
        ...
        <span style={{fontFamily:'Rajdhani',fontSize:18,fontWeight:800,letterSpacing:3,color:'#f5c842'}}>
          ♛ LUDO
        </span>
        ...
        <div style={{
          ...
          fontFamily:'Rajdhani',fontSize:13,fontWeight:700,color:activeHex,
        }}>
          <div style={{width:7,height:7,borderRadius:'50%',background:activeHex,
            animation:'pulse 1.3s ease infinite'}}/>
          {cur?.username}{cur?.isBot?' 🤖':''}'s turn
          {movable.length>0&&diceValue&&!animating&&
            <span style={{fontSize:10,opacity:.7,marginLeft:4}}>· pick token</span>}
          {animating&&<span style={{fontSize:10,opacity:.7,marginLeft:4}}>· moving…</span>}
        </div>

        <div style={{display:'flex',gap:6,alignItems:'center'}}>
          <span style={{fontSize:11,color:'rgba(255,255,255,0.45)',fontFamily:'Rajdhani',fontWeight:600}}>
            ⏱ {formatTime(elapsed)}
          </span>
          {/* "Chats" button */}
          <button onClick={()=>setChatOpen(o=>!o)} style={{
            background:chatOpen?'rgba(26,143,232,.22)':'rgba(255,255,255,0.1)',
            ...
            {state.chatMessages.length>0&&
              <span style={{background:'#1a8fe8',color:'white',borderRadius:'50%',
                ...
                fontSize:9,fontWeight:800,lineHeight:1}}>{state.chatMessages.length}</span>}
          </button>
          <button onClick={()=>{if(confirm('Leave game?'))dispatch({type:'RESET'});}} style={{
      ...
      <div style={{
        ...
        {chatOpen && (
          <div style={{
            position:'fixed',right:0,top:0,bottom:0,width:275,
              ...
              {state.chatMessages.map(m=>(
                <div key={m.id} style={{display:'flex',gap:7,
                  ...
                  <div style={{width:26,height:26,borderRadius:'50%',
                    ...
                    fontSize:10,fontWeight:700,color:'white'}}>{m.sender[0].toUpperCase()}</div>
                  <div style={{maxWidth:'72%'}}>
                    <div style={{fontSize:9,color:Z[m.color]||'#888',fontWeight:700,marginBottom:2,
                      textAlign:m.sender===state.username?'right':'left'}}>{m.sender}</div>
                    <div style={{background:'rgba(255,255,255,0.08)',borderRadius:8,
                      padding:'5px 9px',fontSize:12,lineHeight:1.4,color:'#e0dce8'}}>{m.text}</div>
            ...
            <div style={{padding:'5px 8px',borderTop:'1px solid rgba(255,255,255,0.07)',
              ...
              {['Good Luck! 🍀','Nice Move! 👏','Oops! 😅','GG! 🎉'].map(q=>(
                <button key={q} onClick={()=>{
                  const myColor=players.find(p=>!p.isBot)?.color||'red';
                  dispatch({type:'SEND_CHAT',v:{id:Date.now(),sender:state.username,
            ...
            <div style={{padding:'7px 10px',borderTop:'1px solid rgba(255,255,255,0.07)',
              display:'flex',gap:6}}>
              ...
              <button onClick={sendChat} style={{
                ...
                fontWeight:800,fontSize:14,color:'#1a1200',cursor:'pointer',
                fontFamily:'Rajdhani',
              }}>→</button>
            </div>
          </div>
