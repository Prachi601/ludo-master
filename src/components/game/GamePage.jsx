  ...
  return (
    <div
      ...
      {isActive && diceValue && !diceRolling && (
        <div className="roll-result" style={{ color: hex }}>
          {diceValue}
        </div>
      )}
    </div>
...
export default function GamePage() {
  ...
  const movable = diceValue
    ? getMovableTokens(cur?.color, tokens, diceValue)
    : [];
  ...
  const handleRoll = useCallback(() => {
    ...
    const iv = setInterval(() => {
      if (++n >= 8) {
        ...
        if (
          getMovableTokens(players[currentTurn]?.color, tokens, val).length ===
          0
        ) {
  ...
  const doMove = useCallback(
    (color, tokenIdx, newPos) => {
      ...
      if (diceValue === 6 && !isHome && !cap)
        showNotif("🎲 Six! Roll again!", "success");
      dispatch({
        type: "MOVE_TOKEN",
        payload: { color, tokenIdx, newPos, captured: cap, extra },
      });
      const nt = {
        ...tokens,
      ...
      if (checkWinner(color, nt))
        setTimeout(
          () => dispatch({ type: "SET_WINNER", payload: cur?.username }),
          500,
        );
    },
  ...
  return (
    <div className="gp-root">
      ...
      <header className="gp-header">
        ...
        <div
          className="gp-turn"
          style={{ color: activeHex, borderColor: activeHex }}>
          <div className="gp-turn-dot" style={{ background: activeHex }} />
          {cur?.username}
          {cur?.isBot ? " 🤖" : ""}'s turn
          {movable.length > 0 && diceValue && (
            <span className="gp-pick"> — pick a token</span>
          )}
        </div>
