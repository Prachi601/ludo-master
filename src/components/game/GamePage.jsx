    ...
    <div
      ...
      <svg
        ...
        {isActive && !rolling && !value && (
          <text x="50" y="64" textAnchor="middle" fontSize="36" fill={hex}>
            🎲
          </text>
        )}
        {!isActive && (
    ...
    <div
      ...
      <div
        style={{
          ...
          background: hex,
          flexShrink: 0,
          boxShadow: isActive ? `0 0 7px ${hex}` : "none",
        }}
      />
      <DiceFace
        color={color}
        ...
        {lastEmoji && (
          <div
            style={{
              ...
              lineHeight: 1,
              pointerEvents: "none",
              filter: `drop-shadow(0 0 5px ${hex})`,
              animation: "popIn .3s cubic-bezier(.34,1.56,.64,1)",
            }}>
            {lastEmoji}
          </div>
  ...
  return (
    <div className="gp-root">
      ...
      <div className="gp-header">
        <span className="gp-logo">♛ LUDO</span>
        <div
          className="gp-turn"
          style={{
            borderColor: activeHex,
            color: activeHex,
            background: `${activeHex}14`,
          }}>
          <div className="gp-turn-dot" style={{ background: activeHex }} />
          {cur?.username}
          {cur?.isBot ? " 🤖" : ""}'s turn
          {animating && <span className="gp-pick">moving…</span>}
          {!animating && movable.length > 0 && diceValue && (
            <span className="gp-pick">pick token</span>
          )}
        </div>
        <div className="gp-actions">
          <button
            ...
            style={{
              borderColor: chatOpen ? "#1a8fe8" : "rgba(255,255,255,0.15)",
              color: chatOpen ? "#5ab4f5" : "#bbb",
              background: chatOpen
                ? "rgba(26,143,232,.22)"
            ...
            💬 Chats
            {state.chatMessages.length > 0 && (
              <span className="gp-chat-badge">{state.chatMessages.length}</span>
            )}
          </button>
          <button
            className="gp-leave"
            onClick={() => {
              if (confirm("Leave?")) dispatch({ type: "RESET" });
            }}>
            ✕
          </button>
        </div>
          ...
          return (
            <div
              ...
              style={{
                ...
                borderColor: isAct ? Z[p.color] : "rgba(255,255,255,0.1)",
              }}>
              <div
                style={{
                  width: 6,
              ...
              />
              <span style={{ color: "#ccc" }}>{p.username}</span>
              <span style={{ color: Z[p.color], fontWeight: 900 }}>
                {home}/4
              </span>
            </div>
          );
      ...
      {chatOpen && (
        <div className="gp-chat-panel">
          ...
          <div className="gp-chat-input-row">
            ...
            />
            <button onClick={sendChat} className="gp-chat-send">
              →
            </button>
          </div>
        </div>
